(function() {
    ...
    form.prototype.submit = function(event) {
        ...
        for (k in $ff) {
            $f = $ff[k];
            if ($f.type != 'text' && $f.type != 'email' && $f.type != 'textarea' && $f.type != 'select-one' && $f.type != 'checkbox' && $f.type != 'file') continue;
            if ($f.disabled) continue;
            if ($f.type == 'text' || $f.type == 'email' || $f.type == 'textarea') $f.value = $f.value.replace(/^\s+/, '').replace(/\s+$/, '');
    ...
    new form('form01', {
        ...
        method: 'ajax',
        hid: 'lname',
        success: 'Thank you! :) Say, \"Hello!\" at support@startupheroics.com',
    });
})();
